...
public class lunch extends meal{
    ...
    public lunch(List<Integer> order){
        super("Lunch", order);
    }
    ...
    private static String isCompliantWithRules(){
        // Each order must contain a main and a side, drinks are limited to 1
        int mainCount = 0;
        int sideCount = 0;
        ...
        for(HashMap.Entry<String, Integer> entry : drinks.entrySet()){
            if(entry.getValue() > 1){
                returnMsg = returnMsg + entry.getKey() + " cannot be ordered more than once, ";
            }
        }
